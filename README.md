...
##### 1.9.2
- New Function: Include all occurrences of the Input Glyph in other scripts (Option in UI); (Showing only the Category `Letter`) – *Note: Currently this only works for Input Glyphs that are not built from components. A function to catch situations like `Idieresis --> Yi-cy & Iotadieresis`is in development.*
